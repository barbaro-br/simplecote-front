...
export function Breadcrumb({ items }: { items: BreadcrumbItem[] }) {
  return (
    <nav aria-label="Trilha de navegação" className="flex flex-wrap items-center gap-1.5 text-sm ui-uppercase">
      {items.map((item, index) => {
        const ehAtual = index === items.length - 1 || !item.to
        return (
          <span key={`${item.label}-${index}`} className="flex items-center gap-1.5">
            {index > 0 && (
              <CaretRight className="size-4 shrink-0 text-muted-foreground/50" aria-hidden />
            )}
            {item.to && !ehAtual ? (
              <Link
                to={item.to}
