...
import { STATUS_ASSINATURA, prazoLabel, rotuloStatus, type NivelPrazo } from './backoffice.schema'
...
const CLASSE_PRAZO: Record<NivelPrazo, string> = {
            ...
            <table className="w-full text-sm min-w-[860px]">
              ...
              <tbody className="divide-y divide-border">
                {!ordenados?.length ? (
                  ...
                  ordenados.map((c) => {
                    const prazo = prazoLabel(c.trialExpiraEm)
                    return (
                      <tr key={c.id} className="transition-colors hover:bg-muted/50">
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${CLASSE_PRAZO[prazo.nivel]}`}
                          >
                            {prazo.texto}
                          </span>
                        </td>
