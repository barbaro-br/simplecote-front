...
A tela SHALL permitir criar um usuário com nome, e-mail, papel (ADMIN ou OPERADOR) e uma **senha inicial de pelo menos 8 caracteres**, digitada num campo protegido. O formulário SHALL exigir nome e e-mail preenchidos, e-mail válido, um papel escolhido e a senha com o tamanho mínimo; SHALL bloquear o envio enquanto a requisição corre; e SHALL exibir a mensagem de erro do backend junto ao formulário quando a criação for rejeitada. Após o cadastro, a lista SHALL incluir o novo usuário.
...
A tela SHALL oferecer uma ação separada para trocar a senha de um usuário, através de um formulário com o campo de nova senha e um campo de confirmação, ambos protegidos. A confirmação SHALL ser feita no cliente — os dois campos precisam ser iguais — e a nova senha SHALL ter pelo menos 8 caracteres antes de a requisição ser enviada. Um erro do backend SHALL aparecer no formulário.
